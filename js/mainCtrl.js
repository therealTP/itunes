...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //this means when you make your iTunes request, you'll need to get back the information, parse it accordingly, then set it to songData on the scope ->

  // need to set initial value to filterOptions
  $scope.filterOptions = {
    filterText: ''
  };
  ...
  $scope.getSongData = function(artist) {
    itunesService.getItunesDataForArtist(artist)
    .then(
      function(data) {
        $scope.songData = data;
      },
